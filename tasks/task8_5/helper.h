...
template< class T >
int sendRecvReplace( MPI_Win win, Matrix::matrix<T>& target, Matrix::matrix<T>& src, MPI_Datatype custType, int targetRank )
{
    MPI_Win_fence( 0, win );
    int res = MPI_Put( src.raw(), 1, custType, targetRank, 0, 1, custType, win );
    MPI_Win_fence( 0, win );
    std::memcpy( src.raw(), target.raw(), src.width() * src.height() * sizeof( T ) );
    return res;
}
