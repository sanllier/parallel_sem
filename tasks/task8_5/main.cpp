...
int main( int argc, char** argv )
{
    ...
    matrix< MATRIX_TYPE > tempBufABlock;
    matrix< MATRIX_TYPE > tempBufBBlock;
    ...
    tempBufABlock = parallelRead< MATRIX_TYPE, MPI_TYPE >( matAFile, myID, sqrtProcNum, sqrtProcNum, coords, MPI_COMM_WORLD, &aHeader );
    tempBufBBlock = parallelRead< MATRIX_TYPE, MPI_TYPE >( matBFile, myID, sqrtProcNum, sqrtProcNum, coords, MPI_COMM_WORLD, &bHeader );
    ...
    MPI_Win aBlockWin = 0;
    MPI_Win bBlockWin = 0;
    MPI_Win_create( tempBufABlock.raw(), aBlock.width() * aBlock.height() * sizeof( MATRIX_TYPE ), sizeof( MATRIX_TYPE ), \
        MPI_INFO_NULL, MPI_COMM_WORLD, &aBlockWin );
    MPI_Win_create( tempBufBBlock.raw(), bBlock.width() * bBlock.height() * sizeof( MATRIX_TYPE ), sizeof( MATRIX_TYPE ), \
        MPI_INFO_NULL, MPI_COMM_WORLD, &bBlockWin );
    ...
    checkres( sendRecvReplace< MATRIX_TYPE >( aBlockWin, tempBufABlock, aBlock, SR_ABLOCK, shiftDst ) );
    ...
    checkres( sendRecvReplace< MATRIX_TYPE >( bBlockWin, tempBufBBlock, bBlock, SR_BBLOCK, shiftDst ) );
    ...
    for ( int i = 0; i < dims[0]; ++i )
    {
        ...
        checkres( sendRecvReplace< MATRIX_TYPE >( aBlockWin, tempBufABlock, aBlock, SR_ABLOCK, rightRank ) );
        //checkres( MPI_Sendrecv_replace( bBlock.raw(), 1, SR_BBLOCK, upRank, SR_TAG, downRank, SR_TAG, gridComm, &status ) );
        checkres( sendRecvReplace< MATRIX_TYPE >( bBlockWin, tempBufBBlock, bBlock, SR_BBLOCK, downRank ) );
    }
